...
默认情况下，关闭或重启 Service Manager 不会停止已启动的服务；下次启动会校验并重接管仍在运行的服务。如果你明确希望退出管理器时同时停止所有托管服务，可以这样启动：
...
- **进程状态恢复**：运行中进程的身份信息持久化到 `logs/runtime-state.json`，服务管理器重启后自动校验并重新接管（显示运行状态、允许停止），界面标记「🔄 重接管」提示
