...
// ============ State ============
let config = { projects: [] };
const processes = new Map();   // serviceId -> { child, pid, startedAt }
...
app.get('/api/statuses', (_req, res) => {
  ...
  for (const [serviceId, proc] of processes) {
    statuses[serviceId] = { status: 'running', pid: proc.pid, startedAt: proc.startedAt };
  }
  res.json(statuses);
});
...
wss.on('connection', (ws) => {
  ...
  for (const [serviceId, proc] of processes) {
    init.data.statuses[serviceId] = { status: 'running', pid: proc.pid, startedAt: proc.startedAt };
  }
